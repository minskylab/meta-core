...
func (cc *CredentialCreate) SetDeploymentID(id uuid.UUID) *CredentialCreate {
	cc.mutation.SetDeploymentID(id)
	return cc
}
...
func (cc *CredentialCreate) check() error {
	...
	if _, ok := cc.mutation.Registry(); !ok {
		return &ValidationError{Name: "registry", err: errors.New(`ent: missing required field "registry"`)}
	}
...
func (cc *CredentialCreate) createSpec() (*Credential, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.Registry(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
	...
	if nodes := cc.mutation.DeploymentIDs(); len(nodes) > 0 {
		...
		_node.deployment_credentials = &nodes[0]
