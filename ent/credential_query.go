...
func (cq *CredentialQuery) QueryDeployment() *DeploymentQuery {
	query := &DeploymentQuery{config: cq.config}
...
func (cq *CredentialQuery) Clone() *CredentialQuery {
	...
	return &CredentialQuery{
		...
		withDeployment: cq.withDeployment.Clone(),
...
func (cq *CredentialQuery) sqlAll(ctx context.Context) ([]*Credential, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [1]bool{
			cq.withDeployment != nil,
		}
	)
	if cq.withDeployment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withDeployment; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Credential)
		for i := range nodes {
			if nodes[i].deployment_credentials == nil {
				continue
			}
			fk := *nodes[i].deployment_credentials
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "deployment_credentials" returned %v`, n.ID)
			}
			for i := range nodes {
