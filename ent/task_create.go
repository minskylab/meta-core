...
func (tc *TaskCreate) SetDeploymentID(id uuid.UUID) *TaskCreate {
	tc.mutation.SetDeploymentID(id)
	return tc
}
...
func (tc *TaskCreate) SetDeployment(d *Deployment) *TaskCreate {
	return tc.SetDeploymentID(d.ID)
}
...
func (tc *TaskCreate) defaults() {
	...
	if _, ok := tc.mutation.Detached(); !ok {
		v := task.DefaultDetached
		tc.mutation.SetDetached(v)
	}
...
func (tc *TaskCreate) createSpec() (*Task, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.Detached(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeBool,
			Value:  value,
	...
	if value, ok := tc.mutation.Environment(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
	...
	if value, ok := tc.mutation.Ports(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
	...
	if value, ok := tc.mutation.Volumes(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
	...
	if value, ok := tc.mutation.Restart(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
	...
	if value, ok := tc.mutation.SecurityOpt(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
			Column: task.FieldSecurityOpt,
		})
		_node.SecurityOpt = value
	}
	if value, ok := tc.mutation.CapAdd(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
	...
	if nodes := tc.mutation.DeploymentIDs(); len(nodes) > 0 {
		...
		_node.deployment_tasks = &nodes[0]
