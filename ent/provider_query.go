...
// Returns a *NotSingularError when exactly one Provider entity is not found.
...
// Returns a *NotSingularError when exactly one Provider ID is not found.
...
func (pq *ProviderQuery) Clone() *ProviderQuery {
	...
	return &ProviderQuery{
		...
		withDeployment: pq.withDeployment.Clone(),
...
func (pq *ProviderQuery) sqlAll(ctx context.Context) ([]*Provider, error) {
	...
	if query := pq.withDeployment; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Provider)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Deployment = []*Deployment{}
		}
		query.withFKs = true
		query.Where(predicate.Deployment(func(s *sql.Selector) {
			s.Where(sql.InValues(provider.DeploymentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.deployment_provider
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "deployment_provider" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "deployment_provider" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Deployment = append(node.Edges.Deployment, n)
		}
	}
