...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withDeployment: tq.withDeployment.Clone(),
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [1]bool{
			tq.withDeployment != nil,
		}
	)
	if tq.withDeployment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withDeployment; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*Task)
		for i := range nodes {
			if nodes[i].deployment_tasks == nil {
				continue
			}
			fk := *nodes[i].deployment_tasks
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "deployment_tasks" returned %v`, n.ID)
			}
			for i := range nodes {
